...
class Topology:
    ...
    def get_path_by_node_name_list(self, node_name_list):
        if len(node_name_list) < 2:
            raise ValueError("node_name_list must >=2")
        path = []
        node_index_list = []
        for node_name in node_name_list:
            node_index_list.append(self.node_list.index(node_name))
        self.router.calculate_adj_matrix_and_predecessor_matrix()
        path.append(node_name_list[0])
        for i in range(len(node_index_list) - 1):
            src_index = node_index_list[i]
            dst_index = node_index_list[i + 1]
            current_path = self.router.get_path_from_src_to_dst(src_index, dst_index)
            for i in range(1, len(current_path)):
                node_in_current_path_index = current_path[i]
                path.append(self.node_list[node_in_current_path_index])
        return path
